...
def worker(queue):
    ...
    while queueFull:
        try:
            ...
            gaysocket = socks.socksocket(socket.AF_INET, socket.SOCK_STREAM) #change to socks.socksocket if using pysocks
        ...
        try:
            gaytransport = paramiko.Transport(gaysocket)
            gaytransport.start_client()
            sshK = gaytransport.get_remote_server_key()
